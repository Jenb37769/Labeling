...
def get_paddle_ocr():
    ...
    if paddle_ocr is None:
        # PaddleOCR has changed its init signature across versions.
        # Keep only kwargs supported by the installed version.
        candidate_kwargs = {
            'lang': 'en',
            'use_angle_cls': False,
            'use_gpu': False,  # avoid conflicts with the torch process
            'show_log': False,
            'max_batch_size': 1024,
            'use_dilation': True,
...
def get_caption_model_processor(model_name, model_name_or_path="Salesforce/blip2-opt-2.7b", device=None):
    ...
    if model_name == "blip2":
        ...
        processor_source = model_name_or_path
        try:
...
def remove_overlap_new(boxes, iou_threshold, ocr_bbox=None):
    '''
    ocr_bbox format: [{'type': 'text', 'bbox':[x,y], 'interactivity':False, 'content':str, 'confidence': float }, ...]
    boxes format: [{'type': 'icon', 'bbox':[x,y], 'interactivity':True, 'content':None, 'confidence': float }, ...]
                ...
                if not box_added:
                    if ocr_labels:
                        filtered_boxes.append({
                            ...
                            'confidence': box1_elem.get('confidence', 0.0),
                        })
                    else:
                        filtered_boxes.append({
                            'type': 'icon',
...
def get_som_labeled_img(image_source: Union[str, Image.Image], model=None, BOX_TRESHOLD=0.01, output_coord_in_ratio=False, ocr_bbox=None, text_scale=0.4, text_padding=5, draw_bbox_config=None, caption_model_processor=None, ocr_text=[], use_local_semantics=True, iou_threshold=0.9,prompt=None, scale_img=False, imgsz=None, batch_size=128):
    ...
    xyxy_elem = [
        ...
        for box, conf in zip(xyxy.tolist(), logits.tolist())
        if int_box_area(box, w, h) > 0
    ]
